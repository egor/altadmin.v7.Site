...
class GalleryItem extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			//array('galleryId, image, imageAlt, imageTitle, position, visibility', 'required'),
            array('galleryId, image, imageAlt, imageTitle, position, visibility', 'safe'),
			array('visibility', 'numerical', 'integerOnly'=>true),
			array('galleryId, position', 'length', 'max'=>10),
			array('image, imageAlt, imageTitle', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, galleryId, image, imageAlt, imageTitle, position, visibility', 'safe', 'on'=>'search'),
		);
	}
