...
<script type="text/javascript" src="<?php echo Yii::app()->theme->baseUrl; ?>/js/images-sortable/jquery.jeditable.mini.js"></script>
<script type="text/javascript" src="<?php echo Yii::app()->theme->baseUrl; ?>/js/images-sortable/jquery.json-2.2.min.js"></script>
<script type="text/javascript" src="<?php echo Yii::app()->theme->baseUrl; ?>/js/images-sortable/init.js"></script>
...
<div id="images-list-block">
    ...
    if ($imgList) {
        ...
        foreach ($imgList as $value) {
            ...
            <tr><td width="310px;">
            ';
            if (file_exists(Yii::getPathOfAlias('webroot') . '/images/' . $folder . '/big/' . $value->image)) {
                echo '<img class="pimages" src="/images/' . $folder . '/big/' . $value->image . '" >';
            } else {
                echo 'Нет фото';
            }
            echo '
                ...
                <input type="text" value="' . $value->imageAlt . '" maxlength="255" id="imageAlt_' . $value->$modelId . '" name="imageAlt_' . $value->$modelId . '" rows="7" class="span9">
                <br />
                <label for="imageTitle_' . $value->$modelId . '" style="float:left; margin-right:10px; width:30px;">Title:</label>                    
                <input type="text" value="' . $value->imageTitle . '" maxlength="255" id="imageTitle_' . $value->$modelId . '" name="imageTitle_' . $value->$modelId . '" rows="7" class="span9">
...
<a href="#" style="float:right;" onclick="saveImageInfo(' . $value->$modelId . '); return false;" class="btn btn-primary">Сохранить</a>
                        ...
                        <a href="#"  onclick="myModalDeleteImageGallery(' . $value->$modelId . ', \'/images/' . $folder . '/small/' . $value->image . '\'); return false;" title="Удалить"><i class="icon-remove"></i></a>
</td></tr></table>                            
</li>';
        }
        echo '</ul>';
